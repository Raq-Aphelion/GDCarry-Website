...
/** Shared service page core — rendered by the per-game wrappers in this
    folder (ServicePageFFXIV, ServicePageWoW, ...), one per explicit
    /boosting/:gameId/:serviceId route. `gameId` comes from the wrapper; only
    `serviceId` is read from the URL. */
export default function ServicePageCore({ gameId }: { gameId: string }) {
  ...
  // Account pages: the accordion is static — always extended, no toggles
  const staticAccordion = !!service.account;
          ...
          <Reveal key={g.name} delay={gi * 80}>
            <div className={gi > 0 || content.gallery?.length ? 'mt-8' : ''}>
              ...
              <div className="card-surface mt-5 grid grid-cols-1 gap-px overflow-hidden rounded-[5px] bg-navy-700/40 sm:grid-cols-2">
                {g.rewards.map((r, ri) => (
                  /* An odd trailing row (e.g. Delivery's single "Transfer")
                     spans both columns instead of leaving a half-empty row */
                  <div
                    key={r.title}
                    className={`bg-navy-850 ${
                      g.rewards.length % 2 === 1 && ri === g.rewards.length - 1 ? 'sm:col-span-2' : ''
                    }`}
                  >
                    <RewardRow r={r} />
                  </div>
          ...
          return (
            <Reveal key={s.title} delay={i * 80}>
              <div className="card-surface rounded-[5px]">
                {staticAccordion ? (
                  /* Account pages: static, always-extended sections — no toggle */
                  <p className="px-5 py-4 text-left font-display text-base font-bold text-white">{s.title}</p>
                ) : (
                  <button
                    onClick={() =>
